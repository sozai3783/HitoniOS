//
//  CricketGameViewController.swift
//  Hiton
//
//  Created by yao on 16/06/2016.
//  Copyright © 2016 apsystem. All rights reserved.
//

import UIKit

class CricketGameViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    */
    ...
    func setup(){
        ...
        nextPlayerTap = UITapGestureRecognizer(target: self, action: #selector(CricketGameViewController.NextPlayerAction))
    }
                ...
                if isNext == false{
                    ...
                    if dartsHit >= 1 && dartsHit <= 3 {
                        ...
                        logic.Logic_Cricket(_str) { (_originalScore, _SDTBM, _status, _video, _sound) in
                            ...
                            switch _status {
                            case 1:
                                self.CalculateCTScore(_originalScore, _SDTB: _SDTBM, _video: _video, _sound: _sound)
                                break
                            case 2:
                                self.CalculateMiss(_originalScore, _SDTB: _SDTBM)
                                break
...
//            if _directNext == true {
    ...
    func CalculateCTScore(_score: Int, _SDTB: String, _video: String, _sound: String){
        var tempVideo = _video
        var tempPlayer: PlayerClass!
        var tempValue: Int!
        switch nowPlayer {
        ...
        CheckingCTLicense(_score, _Player: tempPlayer, _SDTB: _SDTB, value: tempValue, _video: tempVideo)
    }
    ...
    func CheckingCTLicense(_Score: Int, _Player: PlayerClass, _SDTB: String, value: Int, _video: String){
        print("Score = \(_Score), value = \(value)")
        var tempBool: Bool!
        var tempHit: Int!
        var tempCount: Int!
        var count = _SDTB == "Triple" ? 3 : _SDTB == "Double" ? 2 : _SDTB == "D-Bull" ? 2 : _SDTB == "S-Bull" ? 1 : 1
        ...
        if tempBool == true {
            ...
            if checkLineClose == false {
                ...
                print("AddScore")
                CTDrawScore(value, _score: tempScore)
                //_Player.Score = _Player.Score + tempScore
                //drawPlayerScore()
            }else{
                drawList(0)
        ...
        let tempCheckWinner = CheckCTWinner()
        if tempCheckWinner == false {
            self.drawDartScore(self.dartsHit, _SDTBM: _SDTB, _originalScore: _Score)
            if self.dartsHit == 4 {
                if round == maxRound && nowPlayer == playerCount {
    ...
    func CTDrawScore(_value: Int, _score: Int){
        var otherPlayerLicense1 = CheckOtherPlayerLicense(1, _value: _value)
        var otherPlayerLicense2 = CheckOtherPlayerLicense(2, _value: _value)
        var otherPlayerLicense3 = CheckOtherPlayerLicense(3, _value: _value)
        var otherPlayerLicense4 = CheckOtherPlayerLicense(4, _value: _value)
        
        if otherPlayerLicense1 == false {
            P1.Score += _score
        }
        
        if otherPlayerLicense2 == false {
            P2.Score += _score
        }
        
        if otherPlayerLicense3 == false {
            P3.Score += _score
        }
        
        if otherPlayerLicense4 == false {
            P4.Score += _score
        }
        self.drawPlayerScore()
    }
    
    func CheckOtherPlayerLicense(_Player: Int, _value: Int) -> Bool{
        var tempPlayer: PlayerClass!
        switch _Player {
        case 1:
            tempPlayer = P1
        ...
        case 0:
            //20
            tempBool = tempPlayer.twentyLicense
            break
        case 1:
            tempBool = tempPlayer.nineteenLicense
            break
        case 2:
            tempBool = tempPlayer.eighteenLicense
            break
        case 3:
            tempBool = tempPlayer.seventeenLicense
            break
        case 4:
            tempBool = tempPlayer.sixteenLicense
            break
        case 5:
            tempBool = tempPlayer.fifteenLicense
            break
        case 6:
            //bull
            tempBool = tempPlayer.bullLicense
            break
        default:
            break
        }
    ...
    func roundEndWinnnerCheck(){
        ...
        case 4:
            if P1.Score > P2.Score && P1.Score > P3.Score && P1.Score > P4.Score  {
                Winner.PlayerName.text = "Player 1"
            }else if P2.Score > P1.Score && P2.Score > P3.Score && P1.Score > P4.Score {
                Winner.PlayerName.text = "Player 2"
